...
def process_image(image_path, task_id):
    ...
    task_status[task_id] = {"status": "in progress", "result": None}
    ...
    if success:
        print(
            f"Wykryto {person_count} osoby na obrazie {image_path}. Wynik zapisano w {output_file}."
        )
    else:
    ...
    return jsonify(
        {
            ...
            "result": task_info.get(
                "result"
            ),  # Wynik może być `None`, jeśli zadanie wciąż trwa
        }
    )
